...
app.post("/levels", async (req, res) => {
  try {
    ...
    const level = await Level.create({ title: req.body.title });
    res.json(level);
  } catch (error) {
...
app.put("/levels/:id", async (req, res) => {
  try {
    ...
    await Level.update({ title: req.body.title }, { where: { id: req.params.id } });
...
app.post("/lessons", async (req, res) => {
  try {
    if (!req.body.title || !req.body.level_id) {
      return res.status(400).json({ error: "Title and level_id required" });
    }
...
app.get("/levels/:id/lessons", async (req, res) => {
  try {
    const lessons = await Lesson.findAll({ where: { level_id: req.params.id } });
    res.json(lessons);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.post("/progress", async (req, res) => {
  try {
    if (!req.body.user_id || !req.body.lesson_id) {
      return res.status(400).json({ error: "user_id and lesson_id required" });
    }
...
app.get("/quiz/:level_id", async (req, res) => {
  try {
    const quiz = await Quiz.findOne({ where: { level_id: req.params.level_id } });
...
app.post("/exam/submit", async (req, res) => {
  try {
    ...
    if (!user_id || !quiz_id || score == null) {
      return res.status(400).json({ error: "user_id, quiz_id and score required" });
    }
    const existing = await Result.findOne({ where: { user_id, quiz_id } });
    if (existing) {
    ...
    res.json({ message: "Result saved", percentage, passed });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
